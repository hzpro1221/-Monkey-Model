...
if __name__ == '__main__':
	...
	for epoch in range(num_eps):
		for i, batch in enumerate(train_dataloader):
			...
			for token in batch["document_ids"]:
				sentence += token.ids
			last_hidden_states = language_model.forward(sentence)

			# Xây dựng mask cho các candidate, chồng nó lên nhau
			# Tạo mask full 0
			candidate_mask = torch.zeros(len(batch["document_candidates"]),batch["document_ids"].length)

			# Thay thế các vị trí có token của span -> 1
			for i, candidate in enumerate(batch["document_candidates"]):
			...
			last_hidden_states_mask = last_hidden_state.unsqueeze(0).repeat(len(batch["document_candidates"]), 1, 1)

			# Nhân candidate mask với last_hidden_states mask
			span_masks = candidate_mask.view(len(batch["document_candidates"]),batch["document_ids"].length, 1).repeat(1, 1, 768) * last_hidden_states_mask # d_model = 768

			# Đưa vào mô hình dự đoán 
			logits = model.forward(spans_masks)

			# Tính loss giữa dự đoán và label
			output = loss(logits, batch["document_labels"]).to(device)
